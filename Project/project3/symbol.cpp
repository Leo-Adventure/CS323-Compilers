...
Node* getNodePop(Node* node){
    //DEBUG_INFO;
    Node* res = node->nodes_queue[0];
    // vector<Node*> new_nodes_queue;
    // new_nodes_queue.push_back(res);
    // for(int i = 1; i < node->nodes_queue.size(); i++){
    //     new_nodes_queue.push_back(node->nodes_queue[i]);
    // }
    // printNodeName(res);
    vector<Node*>::iterator k = node->nodes_queue.begin();
	node->nodes_queue.erase(k);
    node->nodes_queue.push_back(res);
    return res;
}