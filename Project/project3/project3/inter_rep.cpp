...
int inter_exp(Node *node, bool single){
    ...
    // INT | FLOAT | CHAR
    if(node->nodes_queue[0]->name.compare("INT") == 0 || node->nodes_queue[0]->name.compare("CHAR") == 0 || node->nodes_queue[0]->name.compare("FLOAT") == 0) {
        // value < 0, means not address
        printf("In irExp, value = %s\n", node->nodes_queue[0]->value);
        AssignTAC *tac = new AssignTAC(tac_vector.size(), -formatPaser(node->nodes_queue[0]->name, node->nodes_queue[0]->value));
        int id = genid(tac);
        return id;
    }
